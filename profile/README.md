# Trabalho PSPD

Este projeto foi desenvolvido como parte da disciplina **Programação para Sistemas Paralelos e Distribuídos (PSPD)**, do curso de **Engenharia de Software** da **Universidade de Brasília (UnB)**, ministrada pelo **Prof. Dr. Fernando W. Cruz**.

## Repositórios

- [grpc-communication-examples](https://github.com/<organization>/grpc-communication-examples): demonstra, com exemplos de aplicações, os tipos de comunicação suportados pelo gRPC, incluindo `unary calls`, `server-streaming calls`, `client-streaming calls` e `bidirectional streaming calls`.

## Integrantes

<table>
	<tr>
		<td align="center">
			<a href="https://github.com/GG555-13">
				<img src="https://github.com/GG555-13.png?size=120" width="96" alt="usuario1" /><br />
				Guilherme Mendes
			</a>
		</td>
		<td align="center">
			<a href="https://github.com/Laisczt">
				<img src="https://github.com/Laisczt.png?size=120" width="96" alt="Laisczt" /><br />
				Laís Paes
			</a>
		</td>
		<td align="center">
			<a href="https://github.com/lcsgborges">
				<img src="https://github.com/lcsgborges.png?size=120" width="96" alt="lcsgborges" /><br />
				Lucas Borges
			</a>
		</td>
		<td align="center">
			<a href="https://github.com/OrlandiRodrigo">
				<img src="https://github.com/OrlandiRodrigo.png?size=120" width="96" alt="OrlandiRodrigo" /><br />
				Rodrigo Orlandi
			</a>
		</td>
		<td align="center">
			<a href="https://github.com/RA-Salles">
				<img src="https://github.com/RA-Salles.png?size=120" width="96" alt="RA-Salles" /><br />
				Ryan Salles
			</a>
		</td>
	</tr>
</table>
